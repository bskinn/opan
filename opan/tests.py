...
class TestOrcaHessBadUsage(SuperOrcaHess):
    ...
    def test_HESS_BadUsageOverwrite(self):
        from opan.error import HessError
        assertErrorAndTypecode(self, HessError, self.oh.load,
...
if __name__ == '__main__':
    ...
    prs.add_argument('--{0}'.format(OLD), action='store_true',
            help="Run tests not yet refactored into submodules")
    ...
    # Run discovery on this file for un-refactored tests, if indicated
    if any_params(params, [OLD]):
        TestMasterSuite.addTest(
                unittest.TestLoader().discover(os.getcwd(), "tests.py"))
