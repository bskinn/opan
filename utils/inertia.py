...
def principals(geom, masses, on_tol=_DEF.Orthonorm_Tol):
    ...
    on_tol   : np.float_, optional
        ...
        within which axes are considered orthonormal.
...
def rot_consts(geom, masses, units=_EURC.InvInertia, on_tol=_DEF.Orthonorm_Tol):
    ...
    'on_tol' is required in order to pass through to the 'principals' function.

    If the system is linear or a single atom, the effectively-zero principal
    moments of inertia will be assigned values of const.PRM.Zero_Moment_Tol
    ...
    0 <= I_A <= I_B <= I_C; the rotational constants calculated from these will
    always be in *decreasing* order as B_A >= B_B >= B_C, retaining the
