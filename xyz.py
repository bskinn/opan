...
class OPAN_XYZ(object):
    """ Container for OpenBabel XYZ data. #RESUME DOCCING

    Contains all geometries present in the OpenBabel file at the path passed
    to __init__. In ORCA, XYZ files contain the highest precision geometry
    ...
    construction from a custom storage container, though using HDF5 for a data
    repository may make this unnecessary. For now, the file-based retrieval
    is the only implemented mode.

    Units of all stored coordinates are in *Bohrs*; however *Angstrom* units
    in input files are assumed by default.

    Instantiation
    -------------
    __init__(XYZ_path, bohrs=False)
        Constructor for an ORCA_XYZ instance, drawing data from an XYZ or TRJ
        file. Optional 'bohrs' parameter permits indicating the units of the
        coordinates, which could be either Bohrs or Angstroms.
    ...
    ------------------
    XYZ_path    : str
        Full path to imported XYZ/TRJ file
    ...
    def _load_file(self, XYZ_path, bohrs=False):
        """ Initialize ORCA_XYZ geometry object from .xyz or .trj file

        Import of an arbitrary number of multiple geometries from an OpenBabel
        XYZ file (one or more). All geometries must have the same number and
        type of atoms, and the ordering of atom types must be retained
        throughout.
        ...
        # Imports
        import numpy as np
        from orca_const import CIC, PHYS, atomNum
        from orca_error import XYZError
        ...
        #  error
        if not ORCA_XYZ.p_geom.search(self.in_str):
        ...
        #  This is fine for Anharmonic ORCA, but may be highly undesirable
        ...
        try:
            self.num_atoms = scast(ORCA_XYZ.p_geom.match(self.in_str)
                                            .group("num"), np.int_)
        ...
        for mch in ORCA_XYZ.p_geom.finditer(self.in_str):
            ...
            for line_mch in ORCA_XYZ.p_coords.finditer(mch.group("coord")):
                ...
                #  temp coordinates vector, CONVERTING TO BOHRS, since ORCA
                #  XYZ output (virtually?) always defaults to Angstroms.
    ...
    def Geom_single(self, g_num):
        """ Retrieve a single geometry from the ORCA_XYZ instance.
    ...
    def Geom_iter(self, g_nums):
        """Iterator over selected geometries from the ORCA_XYZ instance.
    ...
    def Dist_single(self, g_num, at_1, at_2):
        """ Retrieve an interatomic distance from the ORCA_XYZ instance.
    ...
    def Angle_single(self, g_num, at_1, at_2, at_3):
        """ Retrieve an atomic angle from the ORCA_XYZ instance.
    ...
    def Dihed_single(self, g_num, at_1, at_2, at_3, at_4):
        """ Retrieve a dihedral/out-of-plane angle from the ORCA_XYZ instance.
        ...
        from scipy.linalg import norm
        from orca_utils import Vector as Vec
        from orca_error import XYZError
    ...
    def Displ_single(self, g_num, at_1, at_2):
        """ Displacement vector between two atoms from an ORCA_XYZ instance.
